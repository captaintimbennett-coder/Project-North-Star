  ...
  return <>
    ...
    <section className="retreat-artists event-detail-artists">
      <Container>
        ...
        {event.artists.length > 0 ? <div className="retreat-artists__grid">{event.artists.slice(0, 3).map((artist) => <Link className="retreat-artist-preview" href={`${basePath}/artists/${artist.slug}`} key={artist.slug}><div className="retreat-artist-preview__image"><img src={artist.cardImage.src} alt={artist.cardImage.alt} /></div><p>{artist.displayName}</p><span>{artist.location || artist.categories.slice(0, 3).join(" · ") || "Participating Artist"}</span></Link>)}</div> : <div className="event-detail-artists__empty"><p>Participating artists will be announced as event approvals are completed.</p></div>}
